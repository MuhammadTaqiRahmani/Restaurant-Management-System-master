...
# Add this route for editing employee data
@app.route("/edit_employee", methods=['POST'])
def edit_employee():
    if request.method == 'POST':
        employee_id = request.form.get('edit_emp_id')
        ...
        with session_scope() as db_session:
            employee = db_session.query(Employees).filter(Employees.s_no == employee_id).first()
            if employee:
                employee.e_name = new_name
                employee.e_role = new_role
                db_session.commit()  # Commit the changes to update the existing employee
                return '', 204  # No Content status code
